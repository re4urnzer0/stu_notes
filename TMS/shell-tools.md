...
- `Mia` means **user's name**
- `Aura` means **machine's name**
- `/home/Mia` means **current working directory** (cwd)
...
Shell will find these programs in special path. Like `env:$Path`(Windows) or `$PATH`(Linux).
...
PS: flags -> no value, option -> has a value
